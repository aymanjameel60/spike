...
export default function VendorReports(){
 ...
 return <Page><PageHeader title="تقارير المبيعات" description={`عمولة Spike الحالية: ${commission}%`} actions={<Button onClick={()=>download(rows)}>تصدير Excel / CSV</Button>}/><Card><FormGrid><Field label="من"><input type="date" value={from} onChange={e=>setFrom(e.target.value)}/></Field><Field label="إلى"><input type="date" value={to} onChange={e=>setTo(e.target.value)}/></Field></FormGrid><div className="spike-kpis"><div className="spike-kpi">المبيعات<b>{money(total,cur)}</b></div><div className="spike-kpi">عمولة Spike<b>{money(com,cur)}</b></div><div className="spike-kpi">صافي المستحق<b>{money(net,cur)}</b></div><div className="spike-kpi">الطلبات<b>{rows.length}</b></div></div></Card><Card className="spike-table-card">{rows.length?<div className="spike-responsive-table"><table className="spike-vendor-table"><thead><tr><th>الطلب</th><th>التاريخ</th><th>الإجمالي</th><th>العمولة</th><th>الصافي</th></tr></thead><tbody>{rows.slice(0,100).map(r=><tr key={r.id}><td>#{r.display_id||r.id}</td><td>{new Date(r.created_at).toLocaleDateString("ar")}</td><td>{money(r.total,cur)}</td><td>{money(r.commission,cur)}</td><td>{money(r.net,cur)}</td></tr>)}</tbody></table></div>:<Empty>لا توجد بيانات ضمن الفترة المحددة.</Empty>}</Card></Page>
}
