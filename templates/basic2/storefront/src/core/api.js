...
async function fetchMercurCollections(){const d=await apiRequest('/store/collections?limit=100');return d.collections||[]}
...
async function fetchMercurOffers(){const d=await apiOptional('/store/offers?limit=200');return d?.offers||[]}
...
function applyOffersToProducts(productsList,offers){
  ...
  productsList.forEach(p=>{
    ...
    p.offers=list;
    // Only fill missing offer ids from the offers endpoint. Price comes from
    // variants.calculated_price returned by /store/products.
    for(const o of list){
      const vid=o.variant_id||o.product_variant?.id;
      const v=p.variants?.find(x=>x.id===vid);
      if(v&&!v.offerId)v.offerId=o.id;
    }
    const selected=p.variants?.find(v=>v.id===p.selectedVariantId)||p.variants?.[0];
    if(selected?.offerId)p.offerId=selected.offerId;
    const first=list[0];
    p.store=first?.seller?.name||p.store;
    p.storeId=first?.seller?.id||p.storeId;
    p.sellerHandle=first?.seller?.handle||p.sellerHandle;
  });
}
...
async function syncMercurAll({silent=false}={}){
  ...
  try{
    const [regions,currencies,cats,collections,tags,sellers,offers,remote]=await Promise.all([
      fetchMercurRegions().catch(()=>[]),fetchMercurCurrencies().catch(()=>[]),fetchMercurCategories().catch(()=>[]),fetchMercurCollections().catch(()=>[]),fetchMercurTags().catch(()=>[]),fetchMercurSellers().catch(()=>[]),fetchMercurOffers().catch(()=>[]),fetchMercurProducts()
    ]);
    state.liveRegions=regions;state.liveCurrencies=currencies;state.liveCollections=collections;state.liveTags=tags;applyLiveCategories(cats);applyOffersToProducts(remote,offers);
    ...
    await Promise.all([syncCustomer({silent:true}),syncRemoteCart({silent:true}),syncOrders({silent:true}),syncWishlist({silent:true})]);
...
async function loginCustomer(email,password){
  ...
  const token=d.token||d.access_token;if(!token)throw new Error('لم يرجع Mercur رمز تسجيل دخول');setCustomerToken(token);state.loggedIn=true;await syncCustomer({silent:true});await syncOrders({silent:true});await syncWishlist({silent:true});return d;
}
...
// ---------- Wishlist ----------
async function syncWishlist({silent=false}={}){if(!customerToken())return null;const d=await apiOptional('/store/wishlist',{auth:true});const w=d?.wishlist||d;const entries=w?.entries||w?.products||[];if(entries?.length){state.favs=new Set(entries.map(x=>x.reference_id||x.product_id||x.product?.id||x.id).filter(Boolean))}return w}
async function setRemoteWishlist(productId,add=true){if(!customerToken())return null;if(add)return apiRequest('/store/wishlist',{method:'POST',auth:true,body:{reference:'product',reference_id:productId}});const d=await apiOptional('/store/wishlist',{auth:true});const wid=d?.wishlist?.id||d?.id;if(wid)return apiRequest(`/store/wishlist/${wid}/product/${productId}`,{method:'DELETE',auth:true});return null}
...
async function chooseCheapestShipping(){
  ...
  // Spike backend chooses one valid seller/profile shipping option per vendor,
  // then applies the cheapest delivery office selected by that vendor.
  try{
    const d=await apiRequest('/store/spike/checkout/shipping',{method:'POST',body:{cart_id:cart.id,city}});
    ...
    state.selectedShippingOption=first?.shipping_option_id||null;
    return {cart:d?.cart||cart,option:first||null,selected:d?.selected||[]};
  }catch(e){
    console.warn('[Spike checkout shipping fallback]',e);
  }
  // Fallback for older backend versions.
  const opts=await fetchShippingOptions();
  if(!opts.length)return {cart,option:null};
  const cheapest=[...opts].sort((a,b)=>Number(a.amount??Infinity)-Number(b.amount??Infinity))[0];
  const d=await apiRequest(`/store/carts/${cart.id}/shipping-methods`,{method:'POST',body:{option_id:cheapest.id}});
  if(d?.cart)syncLocalCartFromRemote(d.cart);state.selectedShippingOption=cheapest.id;return {cart:d?.cart||cart,option:cheapest};
}
...
async function recoverCompletedCartOrder(cartId){if(!cartId)return null;const d=await apiOptional(`/store/orders?cart_id=${encodeURIComponent(cartId)}&limit=5&fields=*items`,{auth:true});const rows=d?.orders||[];return rows.find(o=>o.cart_id===cartId||o.cart?.id===cartId)||rows[0]||null}
async function finishRecoveredCheckout(order,cartId){if(!order?.id)return null;state.lastPlacedOrder=order;if(!cartId||remoteCartId()===cartId)setRemoteCartId('');state.remoteCart=null;state.cartItems=[];await syncOrders({silent:true});return order}
async function completeRemoteCart(){const cart=await ensureRemoteCart();const cartId=cart.id;try{await prepareRemoteCheckout();const d=await apiRequest(`/store/carts/${cartId}/complete`,{method:'POST'});if(d.type==='order'||d.order)return finishRecoveredCheckout(d.order,cartId);throw new Error(d.error?.message||'تعذر إكمال الطلب بعد تهيئة الشحن والدفع')}catch(error){if(isCompletedCartError(error)){const existing=await recoverCompletedCartOrder(cartId).catch(()=>null);if(existing)return finishRecoveredCheckout(existing,cartId);setRemoteCartId('');state.remoteCart=null;throw new Error('تم إكمال هذه السلة مسبقًا. حدّث الطلبات قبل إعادة المحاولة حتى لا يتم إنشاء طلب مكرر.')}throw error}}
