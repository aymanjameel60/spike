...
app.addEventListener('click',async e=>{
  ...
  if(act){
    ...
    else if(a==='mark-all-read'){state.notifications.forEach(n=>n.read=true);state.notificationMenu=false;showToast('تم تحديد الكل كمقروء');render()}
    else if(a==='clear-notifications'){state.notifications=[];state.notificationMenu=false;showToast('تم مسح الإشعارات');render()}
    else if(a==='open-notification'){const n=state.notifications.find(x=>x.id===act.dataset.id);if(n){n.read=true;go(n.route||'home')}}
...
app.addEventListener('change',e=>{
  if(e.target.name==='pay')state.payment=e.target.value;
...
async function saveAddressFromForm(){
  const details=(document.getElementById('address-line1')?.value||'').trim();const line2=(document.getElementById('address-line2')?.value||'').trim();const landmark=(document.getElementById('address-landmark')?.value||'').trim();const phone=(document.getElementById('address-phone')?.value||'').trim();if(!details){showToast('أدخل سطر العنوان الأول');return}
  let addr;if(state.editingAddressId){addr=state.addresses.find(x=>x.id===state.editingAddressId);if(addr)Object.assign(addr,{details,line2,landmark,phone,type:state.addressFormType})}else{addr={id:'a'+Date.now(),label:'صنعاء',area:'عنوان جديد',details,line2,landmark,phone,type:state.addressFormType};state.addresses.push(addr)}
...
// Typography policy: every element rendered at exactly 12px uses Bold (700).
function applyTwelvePxMedium(root=document){
  root.querySelectorAll('*').forEach(el=>{
    if(getComputedStyle(el).fontSize==='12px') el.classList.add('font-12-medium');
  });
}
const twelvePxObserver=new MutationObserver(()=>requestAnimationFrame(()=>applyTwelvePxMedium()));
twelvePxObserver.observe(document.documentElement,{childList:true,subtree:true});
document.addEventListener('DOMContentLoaded',()=>applyTwelvePxMedium());
